...
public class ChatGptService {
    ...
    public Mono<String> generateRecipe(List<FoodItem> foodItems) {
        ...
        String food = foodItems.stream()
                .map(item -> String.format("%s %s) - Amount: %d, Validity: %s",
                        item.getName(), item.getCategory(), item.getAmount(), item.getValidity()))
                .collect(Collectors.joining("\n"));

        String prompt = "Baseado no meu banco de dados, faça uma receita com seguintes itens: " + food;
